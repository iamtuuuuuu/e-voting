...
export const initializeMint = async (decimals: number, token: web3.Keypair, provider: AnchorProvider) => {
  const ix = await splProgram.account.mint.createInstruction(token)
  const tx = new web3.Transaction().add(ix)

  await provider.sendAndConfirm(tx, [token])
  return await splProgram.rpc.initializeMint(decimals, provider.wallet.publicKey, provider.wallet.publicKey, {
    accounts: {
